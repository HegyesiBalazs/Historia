...
if (!email) {
    alert("Hiba: Email cím nem található!");
    return;
}
...
fetch("hitelesites_api.php", {
    ...
    .then((data) => {
        if (data.success) {
            alert("Sikeres hitelesítés! Jelentkezz be!");
            localStorage.removeItem("reg_email"); // Email törlése
            hitelesitesModal.classList.add("hidden");
            div2.classList.remove("hidden"); // Bejelentkezés modal megnyitása
        } else {
            alert(data.message || "Érvénytelen vagy lejárt kód!");
        }
    })
    .catch(() => alert("Hiba történt a kérés során!"));
});
