...
function updateMap(year) {
  ...
  // Alapértelmezett bejelentkezési állapot
let isLoggedIn = false;

// Bejelentkezési állapot lekérdezése a szerverről
fetch('bejelentkezett.php')
    .then(response => response.text())
    .then(data => {
        isLoggedIn = data === "true";
        updateJegyzetState(); // Frissíti a jegyzet állapotát az oldal betöltésekor
    })
    .catch(error => console.error('Hiba a bejelentkezési állapot lekérdezésekor:', error));

function updateJegyzetState() {
...
function toggleJegyzet() {
    ...
    if (jegyzet.classList.contains('active')) {
        ...
        jegyzet.classList.remove('active');
    } else {
        overlay.style.display = 'block';
        jegyzet.classList.add('active');
        if (isLoggedIn) {
            content.style.display = 'block';
            loginMessage.style.display = 'none';
        } else {
...
function saveNotes() {
    ...
    const jegyzet = document.getElementById('jegyzetSzoveg').value;

    fetch('jegyzetmentes.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'jegyzet=' + encodeURIComponent(jegyzet)
    })
    .then(response => response.text())
    .then(data => {
        if (data.startsWith('success:')) {
            ...
            alert(data.replace('error:', 'Hiba: '));
        }
    })
    .catch(error => console.error('Hiba a jegyzet mentésekor:', error));
}

function sendNotes() {
    if (!isLoggedIn) return;

    fetch('jegyzetkuldes.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        }
    ...
    .then(data => {
        if (data.startsWith('success:')) {
            alert(data.replace('success:', ''));
            document.getElementById('jegyzetSzoveg').value = ''; // Törli a szövegmezőt
        } else {
            alert(data.replace('error:', 'Hiba: '));
        }
    })
    .catch(error => console.error('Hiba a jegyzet küldésekor:', error));
}


// To Do Lista inicializálása
